    ...
    <div className="space-y-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {data.map((agenda) => (
                <tr key={agenda.id} className="hover:bg-slate-50/80 align-top">
                  <td className="px-6 py-4">
                    <p className="font-bold text-slate-900">{agenda.judul}</p>
                    {agenda.deskripsi && <p className="mt-1 text-xs text-slate-500 line-clamp-2">{agenda.deskripsi}</p>}
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    {agenda.isBerulang ? (
                      ...
                      <span className="inline-block px-3 py-1 text-xs font-bold text-slate-400">TIDAK</span>
                    )}
                  ...
                  <td className="px-6 py-4 flex justify-end gap-2">
                    <button onClick={() => handleOpenModal(agenda)} className="rounded-full bg-slate-100 p-2 text-slate-600 hover:bg-slate-200 hover:text-emerald-700">
                      <Edit2 className="h-4 w-4" />
                    </button>
                    <button onClick={() => handleDelete(agenda.id)} className="rounded-full bg-slate-100 p-2 text-slate-600 hover:bg-rose-100 hover:text-rose-700">
              ...
              {data.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-slate-500">Belum ada agenda tersimpan.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="w-full max-w-xl rounded-3xl bg-white shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 sticky top-0 bg-white/90 backdrop-blur-sm z-10">
              <h3 className="font-bold text-slate-800 text-lg">{editId ? "Edit Agenda" : "Tambah Agenda"}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              ...
              <label className="block space-y-1.5 text-sm font-semibold text-slate-700">
                ...
                <textarea
                  ...
                  className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none focus:border-emerald-500 focus:bg-white resize-none"
                />
              </label>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-2 border-t border-indigo-100/60">
                    <label className="block space-y-1.5 text-sm font-semibold text-slate-700">
                      ...
                      <select
                        ...
                        className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none focus:border-indigo-500"
                      >
